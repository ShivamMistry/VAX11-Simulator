...
namespace VAX11Environment
{
	...
	public class frmEditor : System.Windows.Forms.UserControl
	{
		...
		private void txtDocument_SelectionChanged(object sender, System.EventArgs e)
		{
			...
			if (OnPositionChange != null && _bDontRefresh != true)
		...
		private int AddColorToTable (ref string ColorTable, Color WantedColor)
		{
			...
			if (ColorIndex < 0)
			{
				// removing the ";}" from the end of the table
				ColorTable = ColorTable.Remove (ColorTable.Length - 3, 2);
				ColorTable += @";\red" + WantedColor.R.ToString() + @"\green" + WantedColor.G.ToString() + @"\blue" + WantedColor.B.ToString();
				ColorTable += @";}";
				return -ColorIndex;
			}
			else
		...
		{
			int iSubString;
			for (int iCurrentLine = 0; (iCurrentLine < LineNumber); ++iCurrentLine)
			{
				//need to check that the LineNumber is valid
				iSubString = oldRtf.IndexOf(@"\par") + (@"\par").Length + 1;
				oldRtf = oldRtf.Remove (0, iSubString);
			}
			iSubString = oldRtf.IndexOf(@"\par") + (@"\par").Length + 1;
		...
		{
			int iSubString;
			for (int iCurrentLine = 0; (iCurrentLine < LineNumber + 1); ++iCurrentLine)
			{
				//need to check that the LineNumber is valid
				iSubString = oldRtf.IndexOf(@"\par") + (@"\par").Length + 1;
				oldRtf = oldRtf.Remove (0, iSubString);
		...
		/// </summary>
		public void ColorBackGroundLine(int LineNumber, Color BkGrnd)
		{
		...
		/// </remarks>
		public void UnColorBackGroundLine(int LineNumber)
		{	
			bool wasSaved = _bDocumentSaved;
			string oldRtf = txtDocument.Rtf;
			string newRtf = GetRTFHeader(ref oldRtf);
			...
			int iSubString = lineRtf.IndexOf(@"\highlight");
			...
			if (lineRtf[0] != '\\') lineRtf = lineRtf.Remove (0,1);
			...
			if (iSubString >= 0) 
			{
				...
				bool bLastWardIsOrder = false;
				while (!bLastWardIsOrder && (sTempLine.EndsWith(" ") || sTempLine.EndsWith(@"\tab") || 
					Regex.IsMatch(sTempLine, @"\\cf[0-9]+$")))
				{
					...
					else
					{
						...
						bLastWardIsOrder = true;
					}
				}
				if (bLastWardIsOrder) newRtf+= " ";
			}
		...
		public string GetTextFromRtf (string Rtf)
		{
		...
		//int lastNumOfLines = 0;   RUNTIME_COMPILER
		...
		public string GetCurrentWord()
		{
			...
			if (iStartPosition < TextSize ) return RealText.Substring(iStartPosition, iEndPosition - iStartPosition + 1);
			else return "";
		}
