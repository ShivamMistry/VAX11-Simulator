...
//				return _RealText;
			...
			if (iLastLineDelWas != -1)
			{
				...
				if (bkGroundNeedReColor)
				{
					if (icurLine == BackGroundLine)
						ColorBackGroundLine(icurLine, Settings.Environment.DEBUG_LINE_COLOR, true);
					else
						ColorBackGroundLine(icurLine, Settings.Environment.BREAKPOINT_COLOR, false);
		...
		private void txtDocument_SelectionChanged(object sender, System.EventArgs e)
		{
			...
			if ((_bStopUpdateLocation == false) && OnPositionChange != null && _bDontRefresh != true)
		...
		private void txtDocument_KeyPress(object sender, System.Windows.Forms.KeyPressEventArgs e)
		{
			...
			// on status bar
			if (OnPositionChange != null && _bDontRefresh != true) OnPositionChange(GetLocationOnRTB(txtDocument));
		...
		{
			// Fix to micfrosoft bug with richtextbox
			if ((e.Control == true && e.KeyCode == Keys.V) || 
				(e.Control == true && e.KeyCode == Keys.L) ||
				(e.KeyCode == Keys.F11) || (e.KeyCode == Keys.F5)
				)
			{
				e.Handled = true;
				return;
		...
		private void InitializeComponent()
		{
			...
			this.txtDocument.ContextMenu = this.contextMenu1;
			...
			this.dotNetMenuProvider1.SetImageIndex(this.mnuPopupUndo, 4);
			...
			this.dotNetMenuProvider1.SetImageIndex(this.mnuPopupRedo, 5);
			...
			// menuItem9
			// 
			this.dotNetMenuProvider1.SetDrawSpecial(this.menuItem9, true);
			...
			this.dotNetMenuProvider1.SetImageIndex(this.mnuPopupDelete, 6);
			...
			this.TempRTB.Location = new System.Drawing.Point(0, 0);
			...
			this.Controls.Add(this.TempRTB);
			this.Name = "frmEditor";
		...
		public frmEditor(Assembler.compileErrorHandler HandleErrorFunction, frmMain mainProg)
		{
			//
			...
			txtDocument.PreparePrinting(mainProg);
		}
		...
		private void UpdateLines(int iFromLine, int iOffset)
		{
			...
			if (BackGroundLine != -1 && iFromLine < BackGroundLine)
				BackGroundLine += iOffset;
			// fix error-line color refrence
			if (ErrorLine != -1 && iFromLine < ErrorLine)
				ErrorLine += iOffset;
		...
		private void ColorLine (int iStartPos, int iEndPos)
		{
			...
			for (int iLoop = 0; iLoop <words; ++iLoop)
			{
				...
				if (cBefore == '.')
				{
					...
					{
						if (CodeProcessor.IsDirective(sWord))
						{
							ColorText (ref CharColorArray,(iStartPos-1)-iLineStart, iLength + 1,
		...
		/// </summary>
		private void ColorBackGroundLine(int LineNumber, Color BkGrnd, bool bTrack)
		{
			if (LineNumber < 1)
				LineNumber = 1;
			...
			_bStopUpdateLocation = false;
			MoveCaretToLine(LineNumber, bTrack);
			txtDocument.Visible = true;
		...
		{	
			...
			MoveCaretToLine(LineNumber, false);
			txtDocument.Visible = true;
		...
		public void MoveCaretToLine (int LineNumber, bool bTrack)
		{
			//if (bTrack) not work until i know how to get the RTB scroller position
			{
				_bStopUpdateLocation = true;
		...
		public void ColorAllBreakPoints()
		{
			...
			while (bList.GetNext()) 
			{
				if (bList.GetCurrentLine() != BackGroundLine)
					ColorBackGroundLine(bList.GetCurrentLine(), Settings.Environment.BREAKPOINT_COLOR, false);
			}
		}
		...
		public void ColorStep (int iLine)
		{
			...
			ColorBackGroundLine(iLine, Settings.Environment.DEBUG_LINE_COLOR, true);
		...
		public void ClearStep()
		{
			...
			if (bList.Contains (BackGroundLine))
				ColorBackGroundLine(BackGroundLine, Settings.Environment.BREAKPOINT_COLOR, false);
			BackGroundLine = -1;
		...
		public void ColorErrorLine (int iLine)
		{
			if (ErrorLine != -1)
			{
				...
				if (bList.Contains(ErrorLine))
					ColorBackGroundLine(ErrorLine, Settings.Environment.BREAKPOINT_COLOR, false);
			}
			if (bList.Contains (iLine))
				UnColorBackGroundLine (iLine);
			ColorBackGroundLine(iLine, Settings.Environment.ERRORS_COLOR, true);
			ErrorLine = iLine;
		}
		...
		public void AddBreakPoint(int iLine, int iPC)
		{
			...
			if ((BackGroundLine != iLine) && (ErrorLine != iLine))
			{
				...
				ColorBackGroundLine(iLine, Settings.Environment.BREAKPOINT_COLOR, false);
